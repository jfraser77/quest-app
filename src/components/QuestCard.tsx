...
export function QuestCard({ quest, isDone, isFlash, onToggle, accentColor }: QuestCardProps) {
...
interface QuestEditorProps {
  quest:     Quest;
  sectionId: string;
  onUpdate:  (sectionId: string, id: string, field: keyof Quest, value: Quest[keyof Quest]) => void;
  onRemove:  (sectionId: string, id: string) => void;
}

export function QuestEditor({ quest, sectionId, onUpdate, onRemove }: QuestEditorProps) {
  const upd = <K extends keyof Quest>(f: K, v: Quest[K]) => onUpdate(sectionId, quest.id, f, v);
      ...
      <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
        <div style={{ flex:1, minWidth:0, display:"flex", flexDirection:"column", gap:8 }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            ...
            <select value={quest.type} onChange={e => upd("type", e.target.value as Quest["type"])} style={{ flex:"1 1 100px" }}>
