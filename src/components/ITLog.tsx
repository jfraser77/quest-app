...
const SEED_LOGS: DayLog[] = [
  {
    id: "d-1", date: "2026-03-12",
    tickets: [
      { id:"t-1", number:"36652", title:"Sync folder issue",  user:"ASC OCALA", status:"in-progress", notes:"Sync tool has been restarted, will be backlogged for some time — review." },
      { id:"t-2", number:"36667", title:"OKTA Application",   user:"Renee T",   status:"resolved",    notes:"Able to login to login.etenet.com — Settings accessible from there." },
    ],
    summary: "", handoff: "",
  },
  {
    id: "d-2", date: "2026-03-11",
    tickets: [
      { id:"t-3", number:"36618", title:"Adobe — PDFs not opening", user:"Amanda Evans", status:"resolved", notes:"When you see the transparent circle, just hover and expand." },
    ],
    summary: "", handoff: "",
  },
  {
    id: "d-3", date: "2026-03-06",
    tickets: [
      { id:"t-4", number:"36299", title:"Phone not working",   user:"",          status:"resolved", notes:"Adjusted sound input." },
      { id:"t-5", number:"36300", title:"Connect tunnel — VPN",user:"April Godoy",status:"resolved", notes:"Unlocked AD." },
    ],
    summary: "", handoff: "Handoff Document — Context Amnesia",
  },
  {
    id: "d-4", date: "2026-03-02",
    tickets: [
      { id:"t-6", number:"35922", title:"Install Software HST PAS", user:"Maddie O", status:"resolved", notes:"Installed." },
      { id:"t-7", number:"36410", title:"Scanner not working",       user:"",         status:"resolved",
        notes:"Disconnect ethernet from printer and reconnect. If on WiFi, restart router. If neither works: right-click printer → Hide Printer → re-add and check if scanner comes back after reconnect." },
    ],
    summary: "", handoff: "",
  },
];

const newId   = (): string => `id-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`;
...
function TicketRow({ ticket, onEdit }: TicketRowProps) {
  ...
  return (
    <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:9, marginBottom:7, overflow:"hidden", transition:"border-color 0.15s" }}>
      <div style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"11px 13px", cursor:"pointer" }}
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:7, flexWrap:"wrap" }}>
            ...
            {ticket.user && <span style={{ fontSize:10, color:"var(--text2)" }}>· {ticket.user}</span>}
          </div>
      ...
      {expanded && (
        <div style={{ padding:"0 13px 12px 31px", borderTop:"1px solid var(--border)" }}>
          ...
          <button className="btn" style={{ marginTop:10, fontSize:11, padding:"5px 12px" }}
...
interface TicketEditorProps {
  ticket:   ITTicket;
  onSave:   (draft: ITTicket) => void;
  onCancel: () => void;
  onDelete: (id: string) => void;
}

function TicketEditor({ ticket, onSave, onCancel, onDelete }: TicketEditorProps) {
  const [draft, setDraft] = useState<ITTicket>({ ...ticket });
  const upd = <K extends keyof ITTicket>(f: K, v: ITTicket[K]) =>
    setDraft(p => ({ ...p, [f]: v }));
  ...
  return (
    <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"14px", marginBottom:10 }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:8, flexWrap:"wrap" }}>
        <input value={draft.number} onChange={e => upd("number", e.target.value)}
          placeholder="Ticket #" style={{ flex:"0 0 100px" }}/>
        <select value={draft.status} onChange={e => upd("status", e.target.value as TicketStatus)}
      ...
      <input value={draft.title} onChange={e => upd("title", e.target.value)}
        placeholder="Issue title..." style={{ marginBottom:8 }}/>
      <input value={draft.user} onChange={e => upd("user", e.target.value)}
        placeholder="User / location (optional)..." style={{ marginBottom:8 }}/>
      <textarea value={draft.notes} onChange={e => upd("notes", e.target.value)}
        placeholder="Notes, steps taken, resolution..." rows={3}
...
interface DayBlockProps {
  day:      DayLog;
  onUpdate: (day: DayLog) => void;
  onDelete: (id: string) => void;
  isToday:  boolean;
}

function DayBlock({ day, onUpdate, onDelete, isToday }: DayBlockProps) {
  const [editingTicket, setEditingTicket] = useState<string | null>(null);
  const [showSummary,   setShowSummary]   = useState<boolean>(isToday);
  const [collapsed,     setCollapsed]     = useState<boolean>(!isToday);

  const resolved = day.tickets.filter(t => t.status === "resolved").length;
  const total    = day.tickets.length;

  const saveTicket = (draft: ITTicket) => {
    const tickets = editingTicket === "new"
      ? [...day.tickets, { ...draft, id: newId() }]
      : day.tickets.map(t => t.id === draft.id ? draft : t);
  ...
  const startNew = () => { setEditingTicket("new"); setCollapsed(false); };
    ...
    <div style={{ marginBottom:18 }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10, cursor:"pointer" }}
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, flex:1 }}>
          ...
          <span style={{ fontSize:10, color:"var(--text3)" }}>{resolved}/{total} resolved</span>
        </div>
      ...
      {!collapsed && (
        <div className="reveal">
          {day.tickets.map(t =>
            editingTicket === t.id ? (
              <TicketEditor key={t.id} ticket={t}
                onSave={saveTicket} onCancel={() => setEditingTicket(null)} onDelete={deleteTicket}/>
            ) : (
          ...
          {editingTicket === "new" && (
            <TicketEditor ticket={{ ...BLANK_TICKET(), id:"new-tmp" }}
              onSave={saveTicket} onCancel={() => setEditingTicket(null)} onDelete={() => {}}/>
          )}

          {editingTicket !== "new" && (
            <button className="btn btn-dashed" onClick={startNew}>+ Add ticket</button>
          )}
          ...
          <div style={{ marginTop:12 }}>
            ...
            {showSummary && (
              <div className="reveal">
                <div style={{ fontSize:10, color:"var(--text3)", letterSpacing:"0.1em", marginBottom:6 }}>DAY SUMMARY</div>
                <textarea value={day.summary}
                  onChange={e => onUpdate({ ...day, summary: e.target.value })}
                  placeholder="What got done, what didn't, any patterns noticed..." rows={3}
                  style={{ resize:"vertical", marginBottom:10 }}/>
                <div style={{ fontSize:10, color:"var(--text3)", letterSpacing:"0.1em", marginBottom:6 }}>HANDOFF NOTES</div>
                <textarea value={day.handoff}
                  onChange={e => onUpdate({ ...day, handoff: e.target.value })}
                  placeholder="Context for tomorrow / next person — what's in flight, what needs follow-up..." rows={3}
...
interface ITLogProps {
  onBack: () => void;
}

export default function ITLog({ onBack }: ITLogProps) {
  ...
  const [view,   setView]   = useState<string>("log");
  const [search, setSearch] = useState<string>("");
  const supabaseLoaded = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      supabase!.from("it_logs")
        .upsert({ player: "Joe", logs }, { onConflict: "player" })
        .then(({ error }) => { if (error) console.error("[QuestApp] it_logs save error:", error); });
    }, 1500);
  }, [logs]);

  const updateDay = (updated: DayLog) => setLogs(p => p.map(d => d.id === updated.id ? updated : d));
  const deleteDay = (id: string)      => setLogs(p => p.filter(d => d.id !== id));
          ...
          {view==="summary" && (
            <div className="reveal">
              <div className="panel" style={{ marginBottom:16 }}>
                ...
                {(Object.entries(STATUS_META) as [TicketStatus, StatusMeta][]).map(([k, v]) => {
